...
original_author="filipelira1"
original_name="confitec_teste"
original_urlname="confitec-teste"
original_description="Awesome confitec_teste created by filipelira1" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/confitec_teste "${name}"
...
then
    ...
    echo "then run 'confitec_teste' or 'python -m confitec_teste'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'confitec_teste' or 'python -m confitec_teste' or 'poetry run confitec_teste'"
fi
